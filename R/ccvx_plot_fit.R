...
ccvx_plot_fit <- function(ccvx.samples, placebo.adjusted = FALSE,
  ...
  y <- pci.ul <- pci.ll <- numeric(length(dose.range.orig))
  delta <- (1 - cred.int.width)/2
  
  if (!placebo.adjusted){
    for(ii in 1:length(pci.ul)) {
      quantiles <- quantile(unlist(ccvx.samples$jags.samples$mu.tilde[ii, , ]), probs = c(delta, .5, cred.int.width + delta))
      pci.ll[ii] <- quantiles[1]
      y[ii] <- quantiles[2]
      pci.ul[ii] <- quantiles[3]
    }
  ...
  if (placebo.adjusted){
    for(ii in 1:length(pci.ul)) {
      quantiles <- quantile(unlist(ccvx.samples$jags.samples$trt.post[ii, , ]), probs = c(delta, .5, cred.int.width + delta))
      pci.ll[ii] <- quantiles[1]
      y[ii] <- quantiles[2]
      pci.ul[ii] <- quantiles[3]
    }
